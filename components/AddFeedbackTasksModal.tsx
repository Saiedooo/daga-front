...
const AddFeedbackTasksModal: React.FC<AddFeedbackTasksModalProps> = ({
  isOpen,
  onClose,
  onSave,
  customers,
  existingTaskInvoiceIds,
}) => {
  ...
  const recentInvoices = useMemo(() => {
    ...
    customers.forEach((customer) => {
      if (!customer.log) return; // حماية إضافية لو log مش موجود

      customer.log.forEach((entry) => {
        if (
          entry.invoiceId &&
          new Date(entry.date) >= sevenDaysAgo &&
          !existingTaskInvoiceIds.has(entry.invoiceId)
        ) {
          invoices.push({
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedInvoices(new Set(recentInvoices.map((i) => i.key)));
    } else {
  ...
  const handleSave = () => {
    ...
    for (const key of selectedInvoices) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-2xl max-h-[90vh] flex flex-col">
        <h2 className="text-xl font-bold mb-2 text-text-primary">
          إضافة فواتير جديدة للمتابعة
        </h2>
        <p className="text-sm text-text-secondary mb-4">
          اختر الفواتير من الأسبوع الماضي التي لم تتم متابعتها بعد.
        </p>
              ...
              <tr>
                <th className="px-4 py-2 w-12">
                  <input
                    type="checkbox"
                    checked={
                      selectedInvoices.size === recentInvoices.length &&
                ...
                recentInvoices.map((invoice) => (
                  <tr key={invoice.key} className="border-b hover:bg-gray-50">
                    <td className="px-4 py-2">
                      <input
                        type="checkbox"
                        checked={selectedInvoices.has(invoice.key)}
                        onChange={() => handleToggleSelection(invoice.key)}
                      />
                    </td>
        ...
        <div className="mt-6 flex justify-end space-x-2 space-x-reverse">
          ...
          <button
            onClick={handleSave}
            className="px-4 py-2 bg-primary text-white rounded-md hover:bg-blue-800 disabled:opacity-50 disabled:cursor-not-allowed"
