...
const ComplaintDetailModal: React.FC<ComplaintDetailModalProps> = ({
  ...
  const canEscalate = ([UserRole.TeamLeader, UserRole.GeneralManager] as UserRole[]).includes(currentUser.role);
  const canDoAdminResolve = ([UserRole.GeneralManager, UserRole.AccountsManager] as UserRole[]).includes(currentUser.role);
