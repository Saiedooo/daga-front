...
const ComplaintModal: React.FC<ComplaintModalProps> = ({
  ...
  const handleSave = () => {
    ...
    if (
      ...
      setError('الرجاء تعبئة الحقول الإلزامية: العميل، نوع ال Shكوى، والوصف.');
      return;
    }

    const selectedCustomer = customers.find(
      (c) => c.id === newComplaint.customerId
    );
    if (!selectedCustomer) {
      setError('العميل المختار غير موجود.');
      return;
    }

    // البيانات الكاملة المطلوبة من الـ backend
    const complaintData = {
      ...
      channel: newComplaint.channel || 'هاتف',
      priority: newComplaint.priority || 'عادية',
      description: newComplaint.description,
      complaintText: newComplaint.description, // مهم جدًا للـ pre('validate')
      dateOpened: new Date().toISOString(), // مهم جدًا
      productId: newComplaint.productId || null,
      productColor: newComplaint.productColor || null,
      productSize: newComplaint.productSize || null,
      attachments: newComplaint.attachments || [],
    };

    console.log('Saving complaint:', complaintData); // للتأكد في Console

    onSave(complaintData);
    onClose();
  };

  const isProductOrInvoiceComplaint =
    newComplaint.type === 'مشكلة في جودة المنتج' ||
    newComplaint.type === 'مشكلة في الفاتورة';
  const isProductComplaint = newComplaint.type === 'مشكلة في جودة المنتج';

  const selectedProduct = useMemo(
    () => products.find((p) => p.id === newComplaint.productId),
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setNewComplaint({
                  ...newComplaint,
                  channel: e.target.value as ComplaintChannel,
                })
              }
              className="w-full p-2 border border-gray-300 rounded-md"
            >
