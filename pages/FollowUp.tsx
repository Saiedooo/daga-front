...
const FollowUpPage: React.FC<FollowUpPageProps> = ({ tasks, users, currentUser, onViewCustomer, showNotification }) => {
  ...
  const handleSaveResolution = async () => {
    ...
    const updatedTask: Partial<FollowUpTask> = { 
    ...
    try {
        await window.electron.followUpTask.update(editingTask.id, { ...updatedTask, version: editingTask.version });
        showNotification('تم إنهاء المهمة بنجاح', 'success');
        setEditingTask(null);
    } catch (e: any) {
        showNotification(e.message || "فشل تحديث المهمة", 'error');
    }
  };
