...
const emptyProduct: Omit<Product, 'id' | 'lastModified' | 'version'> = {
    code: '',
    name: '',
    price: 0,
    cost: 0,
...
interface ProductModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSave: (product: Partial<Product> & { version?: number }) => void;
...
const ProductModal: React.FC<ProductModalProps> = ({ isOpen, onClose, onSave, productToEdit }) => {
    ...
    const validateAndSave = () => {
        ...
        if ((productData.price || 0) <= 0) errs.push('السعر يجب أن يكون أكبر من صفر.');
                             ...
                             <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div className="md:col-span-2">
                                    <div className="flex justify-between items-center mb-2">
                                        ...
                                        {productData.variations && productData.variations.length > 1 && <button onClick={() => removeVariation(varIndex)} className="text-red-500"><TrashIcon className="w-5 h-5"/></button>}
                                    </div>
...
const ProductCard: React.FC<{ product: Product; onEdit: () => void; onDelete: () => void }> = ({ product, onEdit, onDelete }) => {
    ...
    // Handle cases where selected variation might become invalid if product updates
    useEffect(() => {
        if (!product.variations.find(v => v.id === selectedVariation.id)) {
            ...
            <div className="p-4 flex-grow flex flex-col">
                <div className="flex-grow">
                    ...
                    <div className="mt-4">
                     ...
                     <div className="mt-2">
                        <label className="text-xs font-medium text-text-secondary">المقاس (المتوفر: {selectedVariation?.stock[selectedSize] || 0})</label>
                        <div className="flex gap-2 mt-1">
                            {SIZES.map(size => (
...
const InventoryPage: React.FC<InventoryPageProps> = ({ products, logUserAction, showNotification }) => {
    ...
    const handleDeleteProduct = async (productId: string) => {
        if (window.confirm('هل أنت متأكد من رغبتك في حذف هذا المنتج؟')) {
            ...
            try {
                await window.electron.product.delete(productId);
    ...
    const handleSaveProduct = async (product: Partial<Product> & { version?: number }) => {
        const isEditing = !!product.id;
        try {
            if (isEditing) {
                await window.electron.product.update(product.id!, product as Product);
                logUserAction(`تعديل المنتج: ${product.name}.`);
                showNotification('تم تعديل المنتج بنجاح', 'success');
            } else {
                await window.electron.product.create(product);
                logUserAction(`إضافة منتج جديد: ${product.name}.`);
                showNotification('تم إضافة المنتج بنجاح', 'success');
            }
        } catch (e: any) {
            showNotification(e.message || "فشل حفظ المنتج", 'error');
        }
    };
