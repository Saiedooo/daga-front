...
import { User, Complaint, Customer, UserRole, ThemeSettings, SystemSettings, Branch } from '../types';
...
const Settings: React.FC<SettingsProps> = ({ currentUser, users, customers, complaints, branches, setActivePage, systemSettings, logUserAction, theme, defaultTheme, showNotification, onRefresh }) => {
  ...
  useEffect(() => {
    // Prevent saving on initial component mount
    ...
    const handler = setTimeout(() => {
      // FIX: window.electron.settings.update does not return a promise.
      // We will assume the save is successful for UI purposes and let the real-time update handle the state.
      window.electron.settings.update({ systemSettings: currentSystemSettings, theme: currentTheme });
      setSaveStatus('saved');
  ...
  }, [currentSystemSettings, currentTheme]);
  ...
  const canAccessDataManagement = isManager;
  const canAccessComplaintTypes = isManager || isTeamLeader;
  const canAccessBranches = isManager || isTeamLeader;
  const canAccessProducts = isManager || isAccountsManager;
  const canAccessCompanyInfo = isManager || isAccountsManager;
  const canAccessEmailSettings = isManager;
  const canAccessTheme = isManager;
  const canAccessClassification = isManager || isAccountsManager;
  const canAccessPoints = isManager || isAccountsManager;
  const canAccessUsers = isManager || isTeamLeader;
  ...
  const handleAddType = () => {
    if (newType && !currentSystemSettings.complaintTypes.includes(newType)) {
      setCurrentSystemSettings(prev => ({ ...prev, complaintTypes: [...prev.complaintTypes, newType] }));
  ...
  const handleDeleteType = (typeToDelete: string) => {
    setCurrentSystemSettings(prev => ({ ...prev, complaintTypes: prev.complaintTypes.filter(type => type !== typeToDelete) }));
  ...
  const handleLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.type.startsWith('image/')) {
      ...
      reader.onloadend = () => {
        setCurrentSystemSettings(prev => ({ ...prev, companyLogo: reader.result as string }));
  ...
  const handleProcessImport = (mapping: { [key: string]: string }) => {
    showNotification('هذه الميزة معطلة حاليًا.', 'info');
  ...
  const handleSaveUser = async (user: User) => {
    ...
    try {
        if (isEditing) {
            await window.electron.user.update(user.id, user);
            logUserAction(`تحديث بيانات الموظف: ${user.name}.`);
            showNotification('تم تحديث بيانات الموظف بنجاح', 'success');
        } else {
            await window.electron.user.create(user);
            logUserAction(`إضافة موظف جديد: ${user.name}.`);
        ...
        showNotification(e.message || 'فشل حفظ الموظف', 'error');
    }
  };
  ...
  const handleDeleteUser = async (userId: string) => {
    ...
    if (window.confirm("هل أنت متأكد من حذف هذا الموظف؟")) {
        try {
            ...
            await window.electron.user.delete(userId);
            logUserAction(`حذف الموظف: ${userName}.`);
            showNotification('تم حذف الموظف بنجاح', 'success');
            onRefresh?.();
        } catch(e: any) {
            showNotification(e.message || 'فشل حذف الموظف', 'error');
        }
    }
  };

  const handleSaveBranch = async (branch: Branch) => {
    const isEditing = branches.some(b => b.id === branch.id);
    try {
        if (isEditing) {
          await window.electron.branch.update(branch.id, branch);
          logUserAction(`تحديث بيانات الفرع: ${branch.name}.`);
          showNotification('تم تحديث بيانات الفرع بنجاح', 'success');
        } else {
          await window.electron.branch.create(branch);
          logUserAction(`إضافة فرع جديد: ${branch.name}.`);
        ...
        showNotification(e.message || 'فشل حفظ الفرع', 'error');
    }
  };
  ...
  const handleDeleteBranch = async (branchId: string) => {
     if (window.confirm("هل أنت متأكد من حذف هذا الفرع؟")) {
        try {
            ...
            await window.electron.branch.delete(branchId);
            logUserAction(`حذف الفرع: ${branchName}.`);
            showNotification('تم حذف الفرع بنجاح', 'success');
            onRefresh?.();
        } catch (e: any) {
            showNotification(e.message || 'فشل حذف الفرع', 'error');
        }
    }
  };
  ...
  const handleThemeFontChange = (value: string) => {
    setCurrentTheme(prev => ({ ...prev, font: value }));
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-background-muted p-4 rounded-lg border border-border">
                ...
                <p className="text-sm text-text-secondary mb-4">
                  (معطل مؤقتاً) قم برفع ملف CSV يحتوي على بيانات العملاء لتحديث قاعدة البيانات.
                </p>
                <input type="file" disabled accept=".csv" className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-primary hover:file:bg-blue-100 disabled:opacity-50"/>
            </div>
            ...
            <div className="bg-red-50 p-4 rounded-lg border border-danger">
                ...
                <p className="text-sm text-red-700 mb-4">
                  (معطل مؤقتاً) تحذير: هذا الإجراء سيقوم بحذف دائم للبيانات.
                </p>
                <div className="flex flex-col sm:flex-row gap-2">
                    <button disabled onClick={() => handleClearData('customers')} className="w-full bg-red-200 text-danger font-semibold px-4 py-2 rounded-md hover:bg-red-300 disabled:opacity-50">حذف العملاء</button>
                    <button disabled onClick={() => handleClearData('complaints')} className="w-full bg-red-200 text-danger font-semibold px-4 py-2 rounded-md hover:bg-red-300 disabled:opacity-50">حذف الشكاوى</button>
                    <button disabled onClick={() => handleClearData('all')} className="w-full bg-danger text-white font-semibold px-4 py-2 rounded-md hover:bg-red-700 disabled:opacity-50">حذف كل شيء</button>
                </div>
            </div>
        </div>
      </AccordionCard>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
              {currentSystemSettings.complaintTypes.map(type => (
        ...
        <AccordionCard title="معلومات الشركة الأساسية">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                <div>
                    ...
                    <input type="text" name="companyName" value={currentSystemSettings.companyName} onChange={handleSettingChange} className="w-full p-2 border border-border rounded-md"/>
                </div>
                <div>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        {currentSystemSettings.companyLogo && <img src={currentSystemSettings.companyLogo} alt="logo preview" className="h-12 w-12 object-contain bg-gray-200 rounded-md p-1"/>}
                    </div>
                </div>
            </div>
        </AccordionCard>
      ...
      {canAccessEmailSettings && (
        <AccordionCard title="إعدادات البريد الإلكتروني (EmailJS)">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <input name="emailJsServiceId" value={currentSystemSettings.emailJsServiceId || ''} onChange={handleSettingChange} placeholder="Service ID" className="p-2 border rounded"/>
                <input name="emailJsTemplateId" value={currentSystemSettings.emailJsTemplateId || ''} onChange={handleSettingChange} placeholder="Template ID" className="p-2 border rounded"/>
                <input name="emailJsPublicKey" value={currentSystemSettings.emailJsPublicKey || ''} onChange={handleSettingChange} placeholder="Public Key (User ID)" className="p-2 border rounded"/>
            </div>
        </AccordionCard>
      )}
            ...
            <p className="text-text-secondary text-sm mb-4">
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" name="silver" value={currentSystemSettings.classification.silver} onChange={(e) => handleSettingChange(e, 'classification')} className="w-full p-2 border border-border rounded-md"/>
                </div>
                 <div>
                    <label className="block text-sm font-medium text-text-secondary mb-1">ذهبي</label>
                    <input type="number" name="gold" value={currentSystemSettings.classification.gold} onChange={(e) => handleSettingChange(e, 'classification')} className="w-full p-2 border border-border rounded-md"/>
                </div>
                 <div>
                    <label className="block text-sm font-medium text-text-secondary mb-1">بلاتيني</label>
                    <input type="number" name="platinum" value={currentSystemSettings.classification.platinum} onChange={(e) => handleSettingChange(e, 'classification')} className="w-full p-2 border border-border rounded-md"/>
                </div>
            </div>
        </AccordionCard>
      )}
        ...
        <AccordionCard title="إعدادات نظام النقاط">
             <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" name="pointValue" value={currentSystemSettings.pointValue} onChange={handleSettingChange} className="w-full p-2 border border-border rounded-md"/>
                </div>
                 <div>
                    <label className="block text-sm font-medium text-text-secondary mb-1">قيمة الشراء للنقاط</label>
                    <input type="number" name="importSpend" value={currentSystemSettings.importSpend} onChange={handleSettingChange} className="w-full p-2 border border-border rounded-md"/>
                </div>
                 <div>
                    <label className="block text-sm font-medium text-text-secondary mb-1">عدد النقاط المكتسبة</label>
                    <input type="number" name="importPoints" value={currentSystemSettings.importPoints} onChange={handleSettingChange} className="w-full p-2 border border-border rounded-md"/>
                </div>
            </div>
             <p className="text-text-secondary text-xs mt-2">
                مثال: يكتسب العميل <span className="font-bold">{currentSystemSettings.importPoints}</span> نقطة مقابل كل <span className="font-bold">{currentSystemSettings.importSpend}</span> جنيه من المشتريات.
            </p>
        </AccordionCard>
      )}
      ...
      {canAccessTheme && (
        <AccordionCard title="تخصيص المظهر والألوان">
            <div className="mb-6 pb-4 border-b">
                ...
                <select value={currentTheme.font} onChange={(e) => handleThemeFontChange(e.target.value)} className="p-2 border border-border rounded-md">
                     ...
                     {colorMappings.filter(c => c.group === group).map(color => (
                        <div key={color.key}>
                            ...
                            <div className="flex items-center gap-2 p-1 border rounded-md">
                                ...
                                />
                                <span className="text-sm font-mono">{currentTheme.colors[color.key]}</span>
                            </div>
                        </div>
                     ))}
                     </div>
            ...
            <button onClick={() => setCurrentTheme(defaultTheme)} className="bg-gray-200 text-gray-800 px-4 py-2 rounded-md hover:bg-gray-300">
