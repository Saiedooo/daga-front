...
const Dashboard: React.FC<DashboardProps> = ({ complaints, users }) => {
  // Safety check: التأكد من أن البيانات arrays
  const safeComplaints = Array.isArray(complaints) ? complaints : [];
  const safeUsers = Array.isArray(users) ? users : [];
    
  const kpiData = useMemo(() => {
    const openComplaints = safeComplaints.filter(c => c.status !== ComplaintStatus.Resolved).length;
    
    const today = new Date().toISOString().slice(0, 10);
    const resolvedToday = safeComplaints.filter(c => c.status === ComplaintStatus.Resolved && c.dateClosed?.startsWith(today)).length;
    
    const resolvedWithTime = safeComplaints.filter(c => c.status === ComplaintStatus.Resolved && c.dateOpened && c.dateClosed);
    ...
    const urgentUnresolved = safeComplaints.filter(c => c.priority === 'عاجلة' && c.status !== ComplaintStatus.Resolved).length;

    return { openComplaints, resolvedToday, avgResolutionHours, urgentUnresolved };
  }, [safeComplaints]);
  
  const complaintsByType = useMemo(() => {
    const counts = safeComplaints.reduce((acc, complaint) => {
  ...
  const performanceByEmployee = useMemo(() => {
    const staffUsers = safeUsers.filter(u => u.role === UserRole.Staff);
    return staffUsers.map(user => {
        const resolvedCount = safeComplaints.filter(c => c.assignedTo === user.id && c.status === ComplaintStatus.Resolved).length;
        return { name: user.name, 'شكاوى تم حلها': resolvedCount };
    });
  }, [safeComplaints, safeUsers]);
